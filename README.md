...
⚙️ Конфигурация среды (.env)Для корректной работы системы в 4-й лабе добавлены важные переменные управления:ПеременнаяОписаниеЗначение для тестаAPP_ENVРежим работы приложенияdevelopment / productionPROJECT_NAMEНазвание в заголовке SwaggerStorage Inventory APIJWT_ACCESS_SECRETСекретный ключ для подписиyour_secret_key
